...
if (typeof(BrowserPonies) !== "object") {
    (function() {
        ...
        if (typeof(console) === "undefined") shim(window, {
            console: {}
        });
        shim(window.console, {
            log: function() {}
    ...
    var BrowserPonies = (function() {
        ...
        var observe = document.addEventListener ? function(element, event, handler) {
            element.addEventListener(event, handler, false);
        } : function(element, event, handler) {
            var wrapper = '_eventHandlingWrapper' in handler ? handler._eventHandlingWrapper : (handler._eventHandlingWrapper = function() {
        ...
        var stopObserving = document.removeEventListener ? function(element, event, handler) {
        ...
        var windowSize = 'innerWidth' in window ? function() {
            return {
        ...
        URL.prototype = {
            toString: function() {
                return this.protocol + '//' + (this.username || this.password ? (this.username || 'anonymous') + (this.password ? ':' + this.password : '') + '@' : '') + this.hostname + (this.port && R4.URL.DEFAULT_PORTS[this.protocol] !== this.port ? ':' + this.port : '') + this.pathname + this.search + this.hash;
            }
        };
        extend(URL, {
            FILE_REGEX: /^(file:)\/\/()()()()([^#\?]*)(\?[^#]*)?(#.*)?$/i,
            ...
            fix: function(url) {
                return url.replace(/^https?:\/\/web\d?\.student\.tuwien\.ac\.at\/~e0427417\/browser-ponies/);
            }
        });
        ...
        var setOpacity = IE && IEVersion[0] < 10 ? function(element, opacity) {
            try {
                element.style.filter = element.style.filter.replace(/\balpha\([^\)]*\)/gi, '') + 'alpha(opacity=' + (Number(opacity) * 100) + ')';
            } catch (e) {}
        ...
        var Interaction = function Interaction(interaction) {
            ...
            for (var i = 0, n = interaction.targets.length; i < n; ++i) {
                ...
                if (!has(ponies, name)) {
                    console.warn("Interaction " + this.name + " of pony " + interaction.pony + " references non-existing pony " + name);
                } else {
            ...
            deref: function(property, pony) {
                ...
                if (name && lower_name !== 'none') {
                    if (has(pony.behaviors_by_name, lower_name)) {
                        ...
                        console.warn(format("%s: Behavior %s of pony %s references non-existing behavior %s.", pony.baseurl, this.name, pony.name, name));
        ...
        var preload = function(load, url, callback) {
            if (has(resources, url)) {
                ...
                load(loader, url, function(success) {
                    ...
                    if (success) {
                        console.log(format('%3.0f%% %d of %d loaded: %s', resource_loaded_count * 100 / resource_count, resource_loaded_count, resource_count, url));
                    } else {
                        console.error(format('%3.0f%% %d of %d load error: %s', resource_loaded_count * 100 / resource_count, resource_loaded_count, resource_count, url));
                    }
        ...
        onprogress(function(resource_loaded_count, resource_count, url) {
            if (showLoadProgress || progressbar) {
                ...
                progressbar.label.innerHTML = format('Загрузка пони&hellip; %d%%', Math.floor(progress * 100));
                ...
                for (var i = 0, n = pony.speeches.length; i < n; ++i) {
                    ...
                    if (speech.name) {
                        ...
                        if (has(this.speeches_by_name, lowername)) {
                            console.warn(format("%s: Speech name %s of pony %s is not unique.", this.baseurl, speech.name, pony.name));
                        } else {
                    ...
                    if ('group' in speech) {
                        if (speech.group !== 0 && !has(this.behaviorgroups, speech.group)) {
                            console.warn(format("%s: Speech %s references unknown behavior group %d.", this.baseurl, speech.name, speech.group));
                        }
                    } else {
                        speech.group = 0;
                    }
            ...
            if ('behaviors' in pony) {
                for (var i = 0, n = pony.behaviors.length; i < n; ++i) {
                    ...
                    if (has(this.behaviors_by_name, lowername)) {
                        console.warn(format("%s: Behavior name %s of pony %s is not unique.", this.baseurl, behavior.name, pony.name));
                    } else {
                    ...
                    for (var j = 0; j < speakevents.length; ++j) {
                        ...
                        if (speechname) {
                            ...
                            if (has(this.speeches_by_name, speechname)) {
                                ...
                                console.warn(format("%s: Behavior %s of pony %s references non-existing speech %s.", this.baseurl, behavior.name, pony.name, behavior[speakevent]));
                    ...
                    if ('group' in behavior) {
                        if (behavior.group !== 0 && !has(this.behaviorgroups, behavior.group)) {
                            console.warn(format("%s: Behavior %s references unknown behavior group %d.", this.baseurl, behavior.name, behavior.group));
                        }
                    } else {
                        behavior.group = 0;
                    }
        ...
        Pony.prototype = {
            ...
            addInteraction: function(interaction) {
                ...
                if (interaction.targets.length === 0) {
                    console.warn("Dropping interaction " + interaction.name + " of pony " + this.name + " because it has no targets.");
                    return false;
                }
                ...
                if (interaction.behaviors.length === 0) {
                    console.warn("Dropping interaction " + interaction.name + " of pony " + this.name + " because it has no common behaviors.");
                    return false;
                }
        ...
        var isOutsideOf = function(rect, area) {
            ...
            return rect.x < wh || rect.y < hh || rect.x + wh > area.width || rect.y + hh > area.height;
        };
        ...
        PonyInstance.prototype = extend(new Instance(), {
            createImage: function() {
                var touch = function(evt) {
                    ...
                    newEvt.initMouseEvent(type, true, true, evt.target.ownerDocument.defaultView, 1, touch.screenX, touch.screenY, touch.clientX, touch.clientY, evt.ctrlKey, evt.altKey, evt.shiftKey, evt.metaKey, 0, null);
                ...
                return tag('img', {
                    ...
                    ondblclick: function() {
                        ...
                        console.log(format('%s does %s%s for %.2f seconds, is at %d x %d and %s. See:', this.pony.name, this.current_behavior.name, this.current_behavior === this.paint_behavior ? '' : ' using ' + this.paint_behavior.name, duration, pos.x, pos.y,
                            (this.following ? 'follows ' + this.following.name() : format('wants to go to %d x %d', this.dest_position.x, this.dest_position.y))), this);
                    }.bind(this),
                    ...
                    onmouseover: function() {
                        if (!this.mouseover) {
                            this.mouseover = true;
                            if (timer !== null && !this.isMouseOverOrDragging() && (this.canMouseOver() || this.canDrag())) {
                                this.nextBehavior(true);
                            }
                        }
                    }.bind(this),
            ...
            isMouseOverOrDragging: function() {
                return this.current_behavior && (this.current_behavior.movement === AllowedMoves.MouseOver || this.current_behavior.movement === AllowedMoves.Dragged);
            },
            canDrag: function() {
            ...
            update: function(currentTime, passedTime, winsize) {
                ...
                if (this.following) {
                    if (this.following.img.parentNode) {
                        ...
                        if (!this.current_behavior.x && !this.current_behavior.y && dist <= curr.width * 0.5) {
                            dest = null;
                        }
                    } else {
                        this.following = null;
                ...
                if (this.interaction_wait <= currentTime && this.pony.interactions.length > 0 && !this.current_interaction) {
                    var sumprob = 0;
                    var interactions = [];
                ...
                if (currentTime >= this.end_time || (this.end_at_dest && this.dest_position.x === pos.x && this.dest_position.y === pos.y)) {
                    this.nextBehavior();
                    return;
                }
            ...
            setFacingRight: Gecko ? function(value) {
            ...
            behave: function(behavior, moveIntoScreen) {
                ...
                var duration = (behavior.minduration + (behavior.maxduration - behavior.minduration) * Math.random());
                ...
                } else if (!spoke && !this.following && !this.current_interaction) {
                ...
                this.setFacingRight(pos.x !== this.dest_position.x ? pos.x <= this.dest_position.x : this.facing_right);
        ...
        return {
            convertPony: function(ini, baseurl) {
                    ...
                    switch (type) {
                                    ...
                                    if (row.length > 19) {
                                        ...
                                        if (row.length > 21) {
                                            ...
                                            if (behavior.dont_repeat_animation) {
                                                console.warn(baseurl + ': behavior ' + behavior.name + ' sets dont_repeat_animation to true, which is not supported by Browser Ponies due to limitations in browsers. ' + 'Please use a GIF that does not loop instead.');
                                            }
                                            if (row[22]) {
                                                behavior.group = parseInt(row[22], 10);
                                                if (isNaN(behavior.group)) {
                                                    delete behavior.group;
                                                    console.warn(baseurl + ': behavior ' + behavior.name + ' references illegal behavior group id: ', row[22]);
                                                }
                                            }
                                        }
                                    }
                        ...
                        case "effect":
                            var effect = {
                                ...
                                dont_repeat_animation: row[12] ? parseBoolean(row[12]) : false
                            };
                            if (effect.dont_repeat_animation) {
                                console.warn(baseurl + ': effect ' + effect.name + ' sets dont_repeat_animation to true, which is not supported by Browser Ponies due to limitations in browsers. ' + 'Please use a GIF that does not loop instead.');
                            }
                            effects.push(effect);
                            break;
                        case "speak":
                            ...
                            if (row.length === 2) {
                                    ...
                                    if (files.length > 0) {
                                        ...
                                        for (var j = 0; j < files.length; ++j) {
                                            ...
                                            if (filetype in speak.files) {
                                                console.warn(baseurl + ': file type ' + filetype + ' of speak line ' + speak.name + ' is not unique.');
                                            }
                                ...
                                if ('group' in speak && isNaN(speak.group)) {
                                    delete speak.group;
                                    console.warn(baseurl + ': speak line ' + speak.name + ' references illegal behavior group id: ', row[5]);
                                }
                            }
                ...
                for (var i = 0, n = effects.length; i < n; ++i) {
                    ...
                    if (!has(behaviors_by_name, behavior)) {
                        console.warn(baseurl + ": Effect " + effect.name + " of pony " + pony.name + " references non-existing behavior " + effect.behavior);
                    } else {
            ...
            start: function() {
                ...
                input.setAttribute("id", "BrowserPoniesClear");
                input.setAttribute("type", "button");
                input.setAttribute("value", "Убрать пони");
                input.setAttribute("style", "position: fixed; bottom: 10px; right: 10px; background: #1a73e8; border: 0; border-radius: 5px; color: #fff; cursor: pointer; height: 30px; width: 110px; padding: 0 10px 0 10px; text-transform: uppercase;");
                input.addEventListener("click", BrowserPonies.stop);
            ...
            stop: function() {
                let input = document.getElementById("BrowserPoniesClear");
                input.remove();
